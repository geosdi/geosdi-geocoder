 ...
 * Component Types</a> for more detail.
 */
@XmlRootElement(name = "addressComponentType")
public enum GCAddressComponentType implements Serializable {
    ...
    /**
     * {@code POSTAL_CODE_SUFFIX} indicates a postal code suffix as used to
     * address postal mail within the country.
     */
    ...
    POST_BOX("post_box"),
    /**
     * {@code POSTAL_TOWN} indicates a grouping of geographic areas, such as
     * locality and sublocality, used for mailing addresses in some countries.
     */
    ...
    /**
     * Indicates an unknown address component type returned by the server. The
     * Java Client for Google Maps Services should be updated to support the new
     * value.
     */
    UNKNOWN("unknown");

    private static Logger log = Logger.getLogger(GCAddressComponentType.class.getName());

    private String addressComponentType;

    private GCAddressComponentType() {
    }

    GCAddressComponentType(String addressComponentType) {
    ...
    public static GCAddressComponentType lookup(String addressComponentType) {
        if (addressComponentType.equalsIgnoreCase(STREET_ADDRESS.toString())) {
            ...
            return POSTAL_CODE;
        } else if (addressComponentType.equalsIgnoreCase(POSTAL_CODE_SUFFIX.toString())) {
